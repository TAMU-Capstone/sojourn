...
def doc(name):
    """Documents live in docs/ in the repository and beside the tools in a
    flat working copy. Resolve either, so the tool is not silently correct in
    one layout and broken in the other."""
...
SPECS = [
    ("Firmware Design Specification", doc("firmware_spec.md")),
    ("Scenario Package Format",       doc("scenario_format.md")),
    ("Introspection API",             doc("introspection_api.md")),
    ("Scenario Author's Guide",       doc("scenario_authoring.md")),
    ("Platform Design (advisory)",    doc("platform_design.md")),
]
...
def write_register(reqs, cites):
    docs = CHARTER.parent / "requirements.md"
    ...
    csv_path = CHARTER.parent / "requirements.csv"
...
def main():
    ...
    if not CHARTER.exists():
        print(f"FAIL  cannot find charter.md (looked in {REPO/'docs'} and {REPO})")
        return 1
    reqs = parse_charter()
